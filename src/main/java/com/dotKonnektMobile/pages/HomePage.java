...
public class HomePage extends BaseClass{
	...
	public void newArrival() throws InterruptedException {
		...
		List<WebElement> AddToCartButton = getDriver().findElements(By.xpath(addToCartButton));
		int i = AddToCartButton.size();
		if (i == n) {
			System.out.println("All " +i+ " AddToCartButton are present");
		} else {
			System.out.println(n - i + " AddToCartButton are not present ");
			softAssert.assertTrue(false, n - i + " AddToCartButton are not present ");
		}
	...
	public void askOurExpert() throws InterruptedException {
		...
		if(bodyText.contains("ASK OUR EXPERTS")) {
		...
		softAssert.assertAll();
			...
			System.out.println("ASK OUR EXPERTS  is not present on the UI" );
		}
	}
			...
			//for (WebElement i : SendAMessage) {
				...
				Log.info("Clicked on the Send a message button");
					...
					String category = "Name *,Email *,Query *";
					 ...
					 Thread.sleep(2000);
					 WebElement SuccessfullMessage = getDriver().findElement(By.xpath("//p[@id='sg-askExpertModalTypography4']"));
	...
	public void aboveAboutSection() throws InterruptedException {
        ...
        Thread.sleep(3000);
		 ...
		 WebElement Image = getDriver().findElement(By.xpath(img1));
	        ...
	        WebElement categoryName = getDriver().findElement(By.xpath(categoryN));
	 ...
	 * WebElement NewArrivalSection =
	 * getDriver().findElement(By.xpath(newArrivalSection));
	 * Action.scrollByVisibilityOfElement(getDriver(), NewArrivalSection);
	 * 
	 * List<WebElement> products1 = getDriver().findElements(By.xpath(allProduct));
	 * int n = products1.size(); System.out.println(n); Random r = new Random(); int
	 * randomValue = r.nextInt(products1.size());//Getting a random value that is
	 * between 0 and (list's size)-1 System.out.println(randomValue); String quick =
	 * quickview+"["+randomValue+"]"; WebElement QuickView =
	 * getDriver().findElement(By.xpath(quick)); Action.click(getDriver(),
	 * QuickView); // System.out.println(QuickView.getText());
	 * Log.info("Successfully clicked on the quick text of product" + randomValue);
	 * 
	 * Log.info("AddToCart of Product Page verification"); WebElement StockCount =
	 * getDriver().findElement(By.xpath(stockCount));
	 * Assert.assertEquals(StockCount.getAttribute("value"), "1",
	 * "Count is not set to 1"); WebElement addToCartIcon =
	 * getDriver().findElement(By.xpath(cartButton)); boolean result =
	 * Action.isDisplayed(getDriver(), addToCartIcon); String elementTxt =
	 * addToCartIcon.getText(); if (elementTxt.equalsIgnoreCase("ADD TO CART") &&
	 * result){ Assert.assertTrue(addToCartIcon.isEnabled(),
	 * "ADD TO CART button is not enabled");
	 * System.out.println("AddTOCart is enabled");
	 * Log.info("Successfully verified AddToCart Message"); //Thread.sleep(2000);
	 * Action.click(getDriver(), addToCartIcon); Thread.sleep(2000);
	 * JavascriptExecutor js = (JavascriptExecutor) getDriver();
	 * js.executeScript("javascript:window.scrollBy(0,0)");
	 * Log.info("Scroll to top"); WebElement cartIcon =
	 * getDriver().findElement(By.xpath(cart2));
	 * Action.scrollByVisibilityOfElement(getDriver(), cartIcon);
	 * Action.click(getDriver(), cartIcon);
	 * Log.info("Successfully Clicked on the Cart button"); Thread.sleep(2000);
	 * WebElement CheckoutBtn = getDriver().findElement(By.xpath(checkoutBtn));
	 * Action.explicitWait(getDriver(), CheckoutBtn, Duration.ofSeconds(10));
	 * Action.click(getDriver(), CheckoutBtn);
	 * Action.explicitWaitbyTitle(getDriver(), "Checkout", Duration.ofSeconds(10));
	 * //Action.explicitWaitbyTitle(getDriver(), "ProductPage",
	 * Duration.ofSeconds(5));
	 * 
	 * Log.
	 * info("Successfully verified the title of the Product page from search Page");
	 * } else if(elementTxt.equalsIgnoreCase("OUT OF STOCK") && result){
	 * Assert.assertTrue(!addToCartIcon.isEnabled(),
	 * "Out of Stock button is enabled"); Log.info("Product is Out of Stock"); }
	 * 
	 * 
	 * else { System.out.println("Cart Icon is not present");
